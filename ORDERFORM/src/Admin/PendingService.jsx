...
function PendingService() {
  ...
  const handleRemarkChange = async (orderId, targetRowIndex, newRemark, rowId = null) => {
    try {
        ...
        prevOrders.map(order => {
          if (order._id === orderId) {
            const updatedRows = order.rows.map((row, index) => {
              const isMatch = (rowId && row._id && row._id.toString() === rowId.toString()) ||
                              (row.originalRowIndex !== undefined ? row.originalRowIndex === targetRowIndex : index === targetRowIndex);
              return isMatch 
                ? { 
                    ...row, 
                    remark: remarkValue,
      ...
      // Send complete data to backend with rowId and targetRowIndex
      const response = await axios.put(
        `/api/pending-services/${orderId}/row/${targetRowIndex}/remark`, 
        { 
          remark: remarkValue,
  ...
  const startEditingRemark = (orderId, rowIndex, currentRemark, rowId = null) => {
    setEditingRemark({ orderId, rowIndex, rowId });
                ...
                {filteredOrders.length === 0 ? (
                  ...
                  filteredOrders.map((order, orderIndex) =>
                    order.rows.map((row, rowIndex) => {
                      ...
                      const previousRowsCount = filteredOrders.slice(0, orderIndex).reduce((sum, o) => sum + (o.rows ? o.rows.length : 0), 0);
                      const sNo = previousRowsCount + rowIndex + 1;
                      ...
                      return (
                        ...
                        >
                          ...
                          <td style={styles.regularTd(columnWidths.remarks)}>
                            {(editingRemark?.orderId === order._id && (editingRemark?.rowId ? editingRemark?.rowId === row._id : editingRemark?.rowIndex === (row.originalRowIndex !== undefined ? row.originalRowIndex : rowIndex))) ? (
                              <div style={styles.remarkEditor}>
                                ...
                                <div style={styles.remarkButtons}>
                                  <button
                                    onClick={() => handleRemarkChange(order._id, row.originalRowIndex !== undefined ? row.originalRowIndex : rowIndex, tempRemark, row._id)}
                              ...
                              <div 
                                onClick={() => startEditingRemark(order._id, row.originalRowIndex !== undefined ? row.originalRowIndex : rowIndex, row.isCompleted ? (row.remark || 'completed') : (row.remark || 'Pending'), row._id)}
